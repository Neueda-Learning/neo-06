...
- `serviceId` is this repo's name with the hyphen removed — `neo-06` → `neo06`.
...
Pushing to `main` publishes `ghcr.io/neueda-learning/neo-06-{backend,frontend}` and **this repo
...
- **The front end's path prefix is baked at build time.** `APP_BASE_PATH=/neo-06` is a
...
- **`backend/` and `frontend/` stay at the repo root** — the system compose builds them by
  path (`./neo-06/backend`). Moving them breaks the whole stack.
