...
class ModuleApplicationTests {
    ...
    void infoReportsIdentityDomainAndWhatIsMocked() throws Exception {
        mvc.perform(get("/info"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.serviceId").value("neo06"))
                .andExpect(jsonPath("$.domain").value("agreement"))
                ...
                .andExpect(jsonPath("$.team").value("Team 06"))
    ...
    void anApplicationIsAcknowledgedProcessedAndReadableBack() throws Exception {
        ...
        // The row UC 00 writes, carried through UC 07's default dial — SILENT since
        // 009-default-esign-silent. The case is generated and sent for signature, and then it
        // WAITS: the mock no longer signs on the customer's behalf, because a signature that
        // arrives ten milliseconds after the agreement was sent leaves no moment in which a
        // customer could be shown it. PENDING is what "sent for signature" actually looks like.
        // Set the dial back to INSTANT (PUT /esign/config) to exercise the auto-modes.
        // Filtered by id, not counted: H2 is shared across the tests in this context, so a size
        // assertion would depend on order.
